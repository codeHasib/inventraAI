            ...
            <div key={i} className="flex items-start gap-3 rounded-lg border border-slate-200 p-3 dark:border-slate-700">
              <div className="min-w-0 flex-1 space-y-2">
                <select value={item.productId} onChange={(e) => { changeItem(i, "productId", e.target.value); setErrors((p) => { const n = { ...p }; delete n[`item_${i}`]; return n; }); }}
                  className="w-full appearance-none rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <input type="number" min="1" value={item.quantity} onChange={(e) => changeItem(i, "quantity", e.target.value)}
                      className={`w-full rounded-lg border bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:bg-gray-800 dark:text-gray-100 ${overStock ? "border-red-500" : "border-gray-300 dark:border-gray-600"}`} />
                    {overStock && <p className="mt-1 text-xs text-red-500">Max: {product?.currentStock}</p>}
                  </div>
                  <div className="flex-1">
                    <label className="mb-1 block text-xs text-slate-500 dark:text-slate-400">Selling Price</label>
                  ...
                  <div className="pt-7"><span className="text-sm font-medium text-gray-900 dark:text-white">${subtotal.toFixed(2)}</span></div>
                </div>
              </div>
              <button type="button" onClick={() => removeItem(i)} className="mt-7 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-red-500 transition-colors hover:bg-red-50 dark:hover:bg-red-900/20">
